...
class ContextualTextClassificationDataset(BaseTextClassification, Dataset):
    """ This represents a dataset that is of the form

    line1###context_line1a###context_line1b###label1

    line2###context_line2a###context_line2b###label2

    line3###context_line3a###context_line3b###label3
    .
    .
    .
    """
    ...
    def get_lines_labels(self) -> (List[LineWithContext], List[Label]):
        lines: List[LineWithContext] = []
        labels: List[Label] = []

        with open(self.filename) as fp:
            for line in fp:
                line, contextA, contextB, label = line.split("###")
                line = line.strip()
                contextA = contextA.strip()
                contextB = contextB.strip()
                label = label.strip()
                line_instance = LineWithContext(text=line, context=[contextA, contextB], tokenizers=self.tokenizers)
    ...
    def __getitem__(self, idx) -> (LineWithContext, Label):
...
class ContextualTextClassificationDatasetManager(DatasetsManager, ClassNursery):
    def __init__(
        self,
        train_filename: str,
        dev_filename: str,
        ...
        self.train_dataset = ContextualTextClassificationDataset(
            filename=self.train_filename, tokenizers=self.tokenizers
        )
        self.dev_dataset = ContextualTextClassificationDataset(
            filename=self.dev_filename, tokenizers=self.tokenizers
        )
        self.test_dataset = ContextualTextClassificationDataset(
            filename=self.test_filename, tokenizers=self.tokenizers
        )

        super(ContextualTextClassificationDatasetManager, self).__init__(
