...
class DocumentClassificationDataset(BaseTextClassification, Dataset):
    ...
    def get_lines_labels(self) -> (List[List[Line]], List[List[Label]]):
        lines: List[Line] = []
        labels: List[Label] = []
        doc_lines: List[List[Label]] = []
        doc_labels: List[List[Label]] = []
    ...
    def __getitem__(self, idx) -> (List[Line], List[Label]):
